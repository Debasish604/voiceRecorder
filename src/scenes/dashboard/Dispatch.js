...
function DispatchDetails() {
  ...
  const fatchedDashboardData = async () => {
    try {
      const response = await apiService.DashboardDispatch();
  ...
  const setSelectedRouting = (data) => {
   navigate(`/dispatch?customer=${data.customer_name}`);
  ...
  return (
    <Box >
      ...
      >
        <label style={{ fontWeight: 'bold', paddingLeft: '20px', paddingTop: '10px', fontSize: '22px' }}>Despatch</label>
              ...
              <TableContainer>
                ...
                <TableHead sx={{ backgroundColor: colors.grey[999] }}>
                    <TableRow>
                      ...
                      <TableCell sx={{ textAlign: 'center' }}>Back Order Rate</TableCell>
                    </TableRow>
                  </TableHead>
                  <TableBody>
                    {customerDetails.top && (customerDetails.top.map((row, index) => (
                      ...
                      >
                        ...
                        <TableCell sx={{ fontWeight: 'bold', color: 'green', textAlign: 'center' }} >
                          {row.BackOrderRate.toFixed(2)}%</TableCell>
                      </TableRow>
                    )))}
                  </TableBody>
                </Table>
            ...
            <Collapse in={openBottomTab} timeout="auto" unmountOnExit>
              <TableContainer>
                ...
                <TableHead sx={{ backgroundColor: colors.grey[999] }}>
                    <TableRow>
                      <TableCell>Customer Name</TableCell>
                      <TableCell sx={{ textAlign: 'center' }}>On Time Delivery Performance</TableCell>
                    </TableRow>
                  </TableHead>
                  <TableBody>
